...
```json
{
  ...
  "riskNotice": {
    "text": "Расчет показывает ориентировочную переплату. Точные условия, полная стоимость займа, комиссии, штрафы и порядок продления нужно проверять в договоре конкретного кредитора перед подписанием."
  }
}
```
...
Важно:
Не использовать сырой JSON как основной интерфейс админки.

`SeoTool.config` и `SeoPage.contentBlocks` могут храниться в базе как JSON, но владелец сайта должен редактировать их через понятные формы.

Для калькулятора сделать поля:
- сумма по умолчанию;
- минимальная сумма;
- максимальная сумма;
- срок по умолчанию;
...
Для чек-листа сделать повторяемый список вопросов:
- текст вопроса;
- варианты ответов;
- баллы;
- совет при слабом ответе;
- кнопки добавить, удалить и переместить.

Для `contentBlocks` не показывать JSON как рабочий редактор. Использовать простые блоки:
- добавить абзац;
- добавить заголовок;
- добавить список;
- добавить callout;
...
Каждый блок редактируется как карточка с полями.

Разработчик должен реализовать mapper:
...
При реализации SEO-инструментов важно:

- не сломать cookie `zk_region_subject_code_v2`;
- не обходить `Offer.restrictedRegionCodes`;
- все будущие блоки, которые выводят офферы, должны использовать регионально отфильтрованный список;
- серверная защита `/go/[slug]` остается обязательной и не заменяется клиентским фильтром.
