...
Статус: этап 8 закрыт 14 июля 2026 года. Пять внешних мониторов, серверный health-check и email-уведомления проверены на реальном тестовом инциденте.
...
Все пять мониторов созданы и работают. Route и health-check установлены, `/api/health/server` возвращает `200 ok`. Уведомления направляются на email `chrk.omsk@gmail.com`. Тестовое уведомление, Down alert и recovery-письмо доставлены; первые письма UptimeRobot попали в спам, после чего отправитель отмечен как не спам.
...
Установка выполнена 14 июля 2026 года. Ручной режим `all` прошёл дважды: диск 39%, RAM 15–16%, load average `0.20` при 2 CPU, контейнеры без рестартов, HTTPS endpoints `200`, в Caddy logs нет `4xx/5xx` и `/go` `429`, локальные и Google Drive-бэкапы совпадают.
...
В UptimeRobot добавлен обычный бесплатный HTTP(s)-монитор `ZaimKarta server health` для `https://zaimkarta.ru/api/health/server` с интервалом 5 минут. Платный `Cron job / Heartbeat monitoring` не используется.
...
2. Проверить входящие и папку «Спам». Если письмо попало в спам, отметить отправителя UptimeRobot как «Не спам».
...
Этап 8 закрыт 14 июля 2026 года. Выполнены все пункты:
...
Для проверки уведомлений health-монитору временно задали отсутствующий URL, не останавливая production. UptimeRobot зарегистрировал Down, после возврата правильного URL — Recovery. Автоматический быстрый health cron подтверждён: endpoint продолжил отвечать `200` после истечения 12-минутного срока ручного marker.
