...
export const APPLICATION_CHECKLIST_CONFIG = {
  ...
  results: [
    {
      ...
      text: "Карточки ниже перестроятся с учетом способа получения и выбранного приоритета.",
    },
    {
      minPercent: 40,
  ...
  riskNotice: {
    text:
      "Чек-лист не является финансовой рекомендацией, не сохраняет ответы и не гарантирует одобрение. Перед заявкой проверьте требования МФО, договор и полную стоимость займа.",
  },
};
